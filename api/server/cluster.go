...
func (c *clusterApi) createPair(w http.ResponseWriter, r *http.Request) {
	pairRequest := &api.CreateClusterPairRequest{}
	method := "createPair"
	...
	err = inst.CreatePair(pairRequest)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}

	json.NewEncoder(w).Encode("Successfully paired with remote cluster")
}

func (c *clusterApi) processPair(w http.ResponseWriter, r *http.Request) {
	processPairRequest := &api.ProcessClusterPairRequest{}
	method := "processPair"
...
func (c *clusterApi) listPairs(w http.ResponseWriter, r *http.Request) {
	method := "listPairs"
...
func (c *clusterApi) deletePair(w http.ResponseWriter, r *http.Request) {
	deletePairRequest := &api.DeleteClusterPairRequest{}
	method := "createPair"

	if err := json.NewDecoder(r.Body).Decode(deletePairRequest); err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	err = inst.DeletePair(deletePairRequest)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
