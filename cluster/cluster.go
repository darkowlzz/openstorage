...
// pairing of multiple clusters
type ClusterListenerPairOps interface {
	// CreatePair is called when we are pairing with another cluster
	CreatePair(self *api.Node, response *api.ProcessClusterPairResponse) error

	// ProcessPairRequest is called when we get a pair request from another cluster
	ProcessPairRequest(self *api.Node, response *api.ProcessClusterPairResponse) error
}
...
type ClusterPair interface {
	...
	CreatePair(*api.CreateClusterPairRequest) error

	// PairProcess handles an incoming pair request from a remote cluster.
	ProcessPairRequest(*api.ProcessClusterPairRequest) (*api.ProcessClusterPairResponse, error)

	// ListPairs returns list of cluster pairs
	ListPairs() (*api.ListClusterPairsResponse, error)

	// DeletePair Delete a pairing
	DeletePair(*api.DeleteClusterPairRequest) error

	// ResetPairToken resets the authentication token.
	ResetPairToken() (*api.ClusterPairTokenResponse, error)

	// GetPairToken gets the authentication token for this cluster.
	GetPairToken() (*api.ClusterPairTokenResponse, error)
}
